...
// to apply the MMC patch
extern unsigned int sec_ibl_ver;
...
static struct sleep_save s5pv310_core_save_NOEPLL[] = {
	SAVE_ITEM(S5P_CLKDIV_LEFTBUS),
	SAVE_ITEM(S5P_CLKGATE_IP_LEFTBUS),
	SAVE_ITEM(S5P_CLKDIV_RIGHTBUS),
	SAVE_ITEM(S5P_CLKGATE_IP_RIGHTBUS),
...
{
	printk(KERN_DEBUG "s5pv310_cpu_prepare sec_ibl_ver = 0x%x\n",sec_ibl_ver);
	if(sec_ibl_ver == 0x444D4D65)
	{
		s3c_pm_do_save(s5pv310_core_save_NOEPLL, ARRAY_SIZE(s5pv310_core_save_NOEPLL));
	}
	else
	{
...
static int s5pv310_pm_resume(struct sys_device *dev)
{
	...
	if (tmp & (0x1 << 31)) {
		...
		if(sec_ibl_ver == 0x444D4D65)
		{
			s3c_pm_do_restore_core(s5pv310_core_save_NOEPLL,
					ARRAY_SIZE(s5pv310_core_save_NOEPLL));
		}
		else
		{
